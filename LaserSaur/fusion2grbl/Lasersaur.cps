/**
  ...
  Modified to satisfy the simplified Grbl subset readable by MSL Lasersaur -AY
  https://github.com/nortd/lasersaur/wiki/gcode
...
description = "MSL Lasersaur linear";
vendor = "grbl";
...
properties = {
  ...
  laserSaur: true, // Output for MSL Lasersaur
  moveSpeed: 8000, // Move speed for MSL Lasersaur
  cutSpeed: 2000, // Cut speed for MSL Lasersaur
...
propertyDefinitions = {
  ...
  laserSaur: {title:"laserSaur", description:"Output gcode for MSL Lasersaur.", group:0, type:"boolean"},
  moveSpeed: {title:"moveSpeed", description:"Moving (not cutting) velocity for MSL Lasersaur.", group:0, type:"number"},
  cutSpeed: {title:"cutSpeed", description:"Cutting velocity for MSL Lasersaur.", group:0, type:"number"},
...
var xyzFormat = createFormat({decimals:(unit == MM ? 1 : 4), trim:false});
...
function onOpen() {
  ...
  if (!properties.laserSaur) {
	...
	writeBlock(gAbsIncModal.format(90)); // Abs coordinates
	writeBlock(mFormat.format(80)); // AirAssist ON
	}
}
...
function onSection() {
  ...
  if (!properties.laserSaur) {
	...
	writeBlock(sOutput.format(power));
	}
...
function onRapid(_x, _y, _z) {
  forceXYZ(); // added to output both X and Y every time
...
function onClose() {
  if (!properties.laserSaur) {
	...
	writeBlock(mFormat.format(81)); // AirAssist off
	writeBlock(gMotionModal.format(0), xOutput.format(0), yOutput.format(0)); // Return to origin
  }
